;(function($){
    ...
    PromoSlider.prototype.initValue = function(){
        var elementW = this.options.width,
            panelWidth = this.promoPanel.outerWidth(true),
        ...
        if (this.options.width === "100%") {
            elementW = clientW;
            panelWidth = clientW;
            mainWidth = panelWidth * this.promoPanel.length;
        }
    ...
    PromoSlider.prototype.sliderPrev = function(){
        ...
        if (!self.promoMain.is(":animated")) {
            if (self.curren === 0) {
                self.promoMain.animate({
                    left: -self.$element.width()*(self.itemLeng-1) + "px",
                },self.options.speed);
    ...
    PromoSlider.prototype.navigateByClick = function(i){
        ...
        if (!self.promoMain.is(":animated")) {
            self.promoMain.animate({
                left: -(self.curren*self.$element.width()) + "px",
            });
        }
        self.prompItem.eq(self.curren).addClass('promo-select')
        .siblings().removeClass('promo-select');
